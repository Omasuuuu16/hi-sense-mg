...
export async function GET(req: NextRequest) {
    ...
    try {
        let products;
        if (category) {
            products = await dbQuery('SELECT * FROM products WHERE LOWER(category) = ? ORDER BY created_at DESC', [category.toLowerCase()]);
        } else {
...
export async function POST(req: NextRequest) {
    try {
        ...
        await dbQuery(
            ...
            [newId, categoryVal, brandVal, model, specs, numericPrice, sectionVal, imageVal]
        );

        console.log(`Product added to database: ${model}`);
        return NextResponse.json({ 
            success: true, 
            product: { id: newId, category: categoryVal, brand: brandVal, model, specs, price: numericPrice, section: sectionVal, image: imageVal } 
        }, { status: 201 });
